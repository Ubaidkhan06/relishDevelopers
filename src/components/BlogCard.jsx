...
export const BlogCard = ({ data }) => {
  ...
  return (
    <Link href={`/blog/${data?.id}`} className="w-screen">
      <div className="w-80 md:w-96">
        ...
        <div className="flex flex-col justify-between h-[230px]">
          <div className=" h-[110px]">
            ...
            >
              <span className="font-bold capitalize">{data?.title}</span>
              <p className="content">{data?.content}</p>
            </div>
          </div>

          <div className="read-more">
            <span className="flex flex-row items-center align-center gap-3 cursor-pointer font-bold mt-5 text-primary">
              <span>Read More</span>
              <svg
                width="25"
                height="12"
